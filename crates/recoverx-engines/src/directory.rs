...
impl DirectoryScanner {
    ...
    ///
    /// If `deleted_after` / `deleted_before` are set (Unix seconds), only files
    /// whose modification time falls within that window are returned.
    /// Files with unreadable metadata are included with a low confidence score.
    pub fn scan(
        dir: &Path,
        session_id: &str,
